...
def display_form():
    ...
    if request.method == "POST":
        ...
        if password != verify_password:
         ...
         if "." not in email_optional  and "@" not in email_optional or len(email_optional)>20 or len(email_optional)<3 :
             email_optional_error = " Needs valid email and no spaces and between 3 and 20 spaces"

         if user_error or pass_error or match_error or email_optional_error:
        ...
        if not user_error and not pass_error and not mail_error and not email_optional_error:
